...
_DEVICE_FLOPS = {
    ...
    "B200": {"default":2.25e15},
    "MI300X": {"default":1336e12},
    "H100": {"default":989e12},
    "H800":  {"default":989e12},
    "H200":  {"default":989e12},
    "A100":  {"default":312e12},
    "A800":  {"default":312e12},
    "L40S":  {"default":362.05e12},
    "L40": {"default":181.05e12},
    "A40": {"default":149.7e12},
    "L20": {"default":119.5e12},
    "H20": {"default":148e12},
    "910B": {"default":354e12},
    "Ascend910": {"default":354e12},
    "RTX 3070 Ti": {"default":21.75e12},
    "NVIDIA GeForce RTX 5090": {"default":209.5e12, "bf16":209.5e12}
}


def get_device_flops(device_name,unit="T", dtype="default"):
    ...
    for key, value in sorted(_DEVICE_FLOPS.items(), key=lambda x: len(x[0]), reverse=True):
        if key in device_name:
            flops = value.get(dtype, value.get("default", float("inf")))
            break

    return unit_convert(flops, unit)


def _estimate_qwen2_flops(config, batch_seqlens, tokens_per_sec):
    ...
    # non-attn per token fwd & bwd flops
    dense_flops_per_token = 6 * dense_N
    ...
    # attn per token fwd & bwd flops
    attn_flops_per_token = 6 * seqlen_square_sum * head_dim * num_attention_heads * num_hidden_layers / tokens_sum

    # all_layer per token fwd & bwd flops
    flops_per_token = dense_flops_per_token + attn_flops_per_token

    # achieved tflops
    flops_achieved = flops_per_token * tokens_per_sec / 1e12
...
ESTIMATE_FUNC = {
    ...
    "qwen": _estimate_qwen2_flops, # in fact, it's also qwen3
...
def estimate_flops(config,  batch_seqlens,tokens_per_sec, **kargs):
    """
    ...
    if config.model_name not in VALID_CONFIG_TYPE:
        print(
            f"Only support config type of {VALID_CONFIG_TYPE}, but got {config.model_name}. MFU will always be "
            f"zero."
        )
        assert 0
    func = ESTIMATE_FUNC.get(config.model_name, _estimate_unknown_flops)
    sig = inspect.signature(func)
    if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
        estimated_flops = func(config, batch_seqlens,tokens_per_sec,**kargs)
    else:
        estimated_flops = func(config, batch_seqlens,tokens_per_sec)
    promised_flops = get_device_flops("NVIDIA GeForce RTX 5090", dtype=config.params_dtype_str)
    mfu = estimated_flops / promised_flops
    return estimated_flops, promised_flops, mfu