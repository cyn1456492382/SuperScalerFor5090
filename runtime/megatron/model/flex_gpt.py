...
def _gpt(num_layers, init_method, output_layer_init_method, num_tokentypes, parallel_output, hidden_dropout, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels, num_query_groups=None):
    op_list = []
    ...
    for i in range(num_layers):
        ...
        use_gqa = num_query_groups is not None and num_query_groups != num_attention_heads
        if use_gqa:
            qkv_name = "enc-attention-gqa-qkv"
            score_name = "enc-attention-gqa-score"
            context_name = "enc-attention-gqa-context"
        else:
            qkv_name = "enc-attention-qkv"
            score_name = "enc-attention-score"
            context_name = "enc-attention-context"
        op_list.append({"name": qkv_name, "op_index": 1 + i * 13 + 1, "init_method": init_method, "attention_type": AttnType.self_attn, "prev_name": "enc-1st-layernorm"})
        op_list.append({"name": score_name, "op_index": 1 + i * 13 + 2, "layer_number": i, "prev_name": f"enc-{qkv_name.split('-')[1]}-{qkv_name.split('-')[2]}"})
        ...
        op_list.append({"name": context_name, "op_index": 1 + i * 13 + 5, "prev_name": "enc-attention-dropout"})
        op_list.append({"name": "enc-attention-dense", "op_index": 1 + i * 13 + 6, "init_method": output_layer_init_method, "dim1_size": kv_channels * num_attention_heads, "dim2_size": hidden_size, "prev_name": context_name})
...
class FlexGPTModel(MegatronModule):
    def __init__(self,
                 ...
                 pre_process=True,
                 post_process=True, 
                 profiling=False, hidden_dropout=0.1, num_layers=0, hidden_size=0, ffn_hidden_size=0, num_attention_heads=0, kv_channels=0, num_query_groups=None):
        ...
        if not profiling:
            ...
            full_op_list = _gpt(num_layers, init_method, output_layer_init_method, num_tokentypes, parallel_output, \
                                args.hidden_dropout, args.hidden_size, args.ffn_hidden_size, args.num_attention_heads, args.kv_channels, num_query_groups=getattr(args, 'num_query_groups', None))
            current_op_list = []
            ...
            self.full_op_list = _gpt(num_layers, init_method, output_layer_init_method, num_tokentypes, parallel_output, \
                                hidden_dropout, hidden_size, ffn_hidden_size, num_attention_heads, kv_channels, num_query_groups=getattr(args, 'num_query_groups', None))
